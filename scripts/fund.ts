...
async function main() {
  ...
  const funder = new hre.ethers.Contract(
    funderDeployment.address,
    funderDeployment.abi,
    deployer
  );
  ...
  const chainFunderDepositories =
    loadConfigResult.data.funderDepositories[chainId];
  ...
  await Promise.all(
    chainFunderDepositories.map(async ({ owner, values }) => {
      ...
      const multicallCalldata = values.map(
        ({ recipient, lowThreshold, highThreshold }, treeValueIndex) => {
          return funder.interface.encodeFunctionData(
            ...
            [
              ...
              hre.ethers.utils.parseUnits(
                lowThreshold.value.toString(),
                lowThreshold.unit
              ),
              hre.ethers.utils.parseUnits(
                highThreshold.value.toString(),
                highThreshold.unit
              ),
            ]
          );
      ...
      const tryStaticMulticallResult = await go(() =>
        funder.callStatic.tryMulticall(multicallCalldata)
      );
      if (!tryStaticMulticallResult.success) {
        console.log(
          "Failed to call funder.callStatic.tryMulticall:",
      ...
      const successfulMulticallCalldata = (successes as boolean[]).reduce(
        (acc, success, index) => {
          if (!success) {
            console.log(
              `Calldata #${index + 1} reverted with message:`,
              decodeRevertString(returndata[index])
            );
            return acc;
          }
      ...
      if (successfulMulticallCalldata.length > 0) {
        ...
        const tryMulticallResult = await go(() =>
          funder.tryMulticall(successfulMulticallCalldata)
        );
        if (!tryMulticallResult.success) {
          console.log(
            "Failed to call funder.tryMulticall:",
            tryMulticallResult.error.message
          );
          return;
        }
