...
async function main() {
  ...
  await Promise.all(
    chainFunderDepositories.map(async ({ owner, values }) => {
      ...
      if (
        (await hre.ethers.provider.getCode(funderDepositoryAddress)) === "0x"
      ) {
        await funder.deployFunderDepository(owner, tree.root);
        console.log("FunderDepository is deployed at", funderDepositoryAddress);
      } else {
        console.log(
          "FunderDepository is already deployed at",
          funderDepositoryAddress
        );
      }

      // Get FunderDepository balance
      const getBalanceResult = await go(
        () => hre.ethers.provider.getBalance(funderDepositoryAddress),
        { totalTimeoutMs: 10_000, retries: 1, attemptTimeoutMs: 4_900 }
      );
      if (!getBalanceResult.success) {
        console.log(
          "Failed to get FunderDepository balance:",
          getBalanceResult.error.message
        );
        return;
      }
      ...
      console.log(
        "FunderDepository current balance:",
      ...
      if (balance.lt(targetBalance)) {
        ...
        if (!sendTransactionResult.success) {
          console.log(
            "Failed to send funds to FunderDepository:",
            sendTransactionResult.error.message
          );
          return;
        }

        console.log(
          `Topped the FunderDepository at ${funderDepositoryAddress} up to 100 ETH`
        );
      }
    })
  );
