...
describe("FunderDepository", function () {
  const deploy = async () => {
    ...
    const FunderDepository = await hre.ethers.getContractFactory(
      "FunderDepository"
    );
    const funderDepository = await FunderDepository.connect(
      roles.funder
    ).deploy(roles.owner.address, root);
  ...
  describe("constructor", function () {
    context("Owner is not zero address", function () {
      context("Root is not zero", function () {
        it("constructs", async function () {
          ...
          expect(await funderDepository.funder()).to.equal(
            roles.funder.address
          );
      ...
      context("Root is zero", function () {
        it("constructs", async function () {
          ...
          const funderDepository = await FunderDepository.deploy(
            roles.owner.address,
          ...
          expect(await funderDepository.owner()).to.equal(roles.owner.address);
          expect(await funderDepository.root()).to.equal(
            hre.ethers.constants.HashZero
          );
        });
      });
    ...
    context("Owner is zero address", function () {
      it("constructs", async function () {
        ...
        expect(await funderDepository.owner()).to.equal(
          hre.ethers.constants.AddressZero
        );
        expect(await funderDepository.root()).to.equal(root);
      });
    ...
    context("Sender is Funder", function () {
      context("Transfer is successful", function () {
        it("withdraws", async function () {
          ...
          await funderDepository
            .connect(roles.funder)
            .withdraw(roles.randomPerson.address, amount);
